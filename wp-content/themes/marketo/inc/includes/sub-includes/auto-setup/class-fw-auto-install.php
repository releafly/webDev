...
class FW_Simple_Auto_Install {
	private $page_slug = 'xpeedstudio_auto_setup';
	private $file_uri;
	private $file_path;
	private $config_path;
	private $view_path;
	...
	private $theme_id;
	private $option_key;
	protected $_prefix = 'xsar';


	public function __construct() {
		$this->file_uri		 = get_template_directory_uri() . '/inc/includes/sub-includes/auto-setup';
		$this->file_path	 = get_template_directory() . '/inc/includes/sub-includes/auto-setup';
	...
	public function item_menu_page() {
		add_theme_page( 'Auto Setup', 'Auto Setup', 'manage_options', $this->page_slug, array(
			$this,
			'auto_setup_page'
		) );
	}
	...
	public function activate_supported_extensions() {
		...
		if ( ! current_user_can( 'manage_options' ) && ! defined( 'FW' ) ) {
			wp_send_json_error( array( 'message' => "Current user can't manage options or the Unyson Plugin is not yet installed" ) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to activate supported extensions. %s ', 'marketo' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate Unyson dependencies. The %s demo content will not be installed %s.', 'marketo' ), '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'marketo' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the Unyson dependencies manually. Note that this page will not be  accessible until you install the theme again.', 'marketo' )
		);
	}
	...
	private function admin_enqueue_scripts() {
		...
		$auto_setup_data = array(
			...
			'messages'                  => array(
				'on_leave_alert'    => esc_html__( 'Attention, the installation process is not yet finished, if leave this page, you lose the information stored on the site!', 'marketo' ),
				'server_problems'   => esc_html__( "Sorry, we've encountered some errors, try to access this page later.", "marketo" ),
				'process_completed' => esc_html__( 'The installation process was completed successfully.', 'marketo' )
			),
	...
	public function render_install_setup( $params ) {
		wp_enqueue_style( 'xsar-auto-setup-css', $this->css_uri . '/styles.css' );
...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
		...
		if ( ! empty( $this->demos ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'messages'     => array(
						'installing'              => __( 'Installing', 'marketo' ),
						'start_install_plugins'   => __( 'Installing required plugins ...', 'marketo' ),
						'finish_install_plugins'  => __( 'Finished installing required plugins', 'marketo' ),
						'start_activate_plugins'  => __( 'Activaiting required plugins ...', 'marketo' ),
						'finish_activate_plugins' => __( 'Finished activaiting required plugins', 'marketo' ),
					)
				)
			);
		}
	...
	public function get_demo_steps() {
		return array(
			'activate-demo-plugins' => array(
				...
				'message'     => esc_html__( 'Activate required demo plugins', 'marketo' ),
			)
		);
	}
		...
		return array_merge(
			...
			array(
				'activate-unyson'           => array(
					...
					'message'     => esc_html__( 'Activating Unyson', 'marketo' ),
				),
				'activate-required-plugins' => array(
					...
					'message'     => esc_html__( 'Activate required plugins', 'marketo' ),
				)
			)
		);
	}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate %s already installed plugins and dependencies %s by Network Administrator. The %s demo content will not be installed %s.', 'marketo' ), '<b>', '</b>', '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'marketo' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'marketo' )
		);
	}
	...
	public function activate_unyson() {
		...
		$response = FW_Plugin_Installer_Helper::activate( array(
			'name' => 'Unyson',
			'slug' => 'unyson'
		) );
	...
	public function activate_plugins( $plugins ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error( 0, __( 'Current user can\'t activate plugins', 'marketo' ) );
		}

		$response = FW_Plugin_Installer_Helper::bulk_activate( $plugins );
		...
		if ( ! empty( $failed_plugins ) ) {
			return new WP_Error( 0, sprintf(
					esc_html__( 'Failed to activate required plugins. %s', 'marketo' ),
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
	...
	public function get_demo_steps() {
		return array(
			...
			'install-demo-plugins'  => array(
				...
				'message'     => esc_html__( 'Install required demo plugins', 'marketo' ),
			),
		);
	}
	...
	public function install_demo_plugins() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. Folder %s is not writable', 'marketo' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		$response = FW_Plugin_Installer_Helper::bulk_install( $_POST['demo_plugins'] );
		...
		if ( ! empty( $failed_plugins ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. %s', 'marketo' ), implode( ', ', $failed_plugins ) ),
			) );
		}
		ob_end_clean();
		wp_send_json_success();
		...
		return array_merge(
			...
			array(
				'install-unyson'               => array(
					...
					'message'     => esc_html__( 'Downloading and installing Unyson', 'marketo' ),
				),
				'install-required-plugins'     => array(
					...
					'message'     => esc_html__( 'Installing required plugins', 'marketo' ),
				),
				'install-supported-extensions' => array(
					...
					'message'     => esc_html__( 'Installing supported extensions', 'marketo' ),
				),
				'install-child-theme'          => array(
					'ajax_action' => $this->_prefix . '_install_child_theme',
					'nonce'       => wp_create_nonce( 'install-child-theme' ),
					'message'     => esc_html__( 'Downloading and installing child theme', 'marketo' ),
				),
			)
		);
	}
	...
	public function install_supported_extensions() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install supported extensions. Folder %s is not writable', 'marketo' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to install supported extensions. %s ', 'marketo' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function install_unyson() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Unyson. Folder %s is not writable', 'marketo' ), WP_PLUGIN_DIR ),
			) );
		}

		$response = FW_Plugin_Installer_Helper::install( array(
			'name' => 'Unyson',
			'slug' => 'unyson'
		) );
	...
	public function install_required_plugins() {
		...
		$response = FW_Plugin_Installer_Helper::bulk_install( $this->get_required_plugins() );
	...
	public function install_child_theme() {
		...
		if ( ! current_user_can( 'install_themes' ) || empty( $this->child_theme_source ) ) {
			wp_send_json_error( array( 'message' => "Current user can't install themes or child theme source must be specified" ) );
		}
		...
		if ( ! $this->initialize_filesystem( $credentials, wp_normalize_path( get_theme_root() ) ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Theme. Folder %s is not writable', 'marketo' ), wp_normalize_path( get_theme_root() ) ),
			) );
		}
		...
		if ( ! $wp_filesystem->is_dir( $child_name ) ) {
			$response = FW_Installer_Helper::download_and_install_a_package(
	...
	public function get_setup_messages() {
		return array(
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies together %s with the demo content %s for the theme.', 'marketo' ), '<b>', '</b>' ),
			'plugins_only'      => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies. The %s demo content will not be installed %s.', 'marketo' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and install all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'marketo' )
		);
	}
}

class FW_Installer_Helper {
	public static function download_and_install_a_package( $package, $destination, $hook_extra = array(), $folder = false ) {
		 ...
		 */
		$upgrader = new WP_Upgrader( new FW_Auto_Install_Upgrader_Skin() );
...
class FW_Plugin_Installer_Helper extends FW_Installer_Helper {
	/**
	...
	public static function install( $plugin, $force = false ) {
		return self::is_installed( $plugin ) && ! $force
			? array(
				'success' => true,
				'data'    => array(
					'message' => esc_html__( 'Plugin is already installed.', 'marketo' )
				)
			)
	...
	public static function activate( $plugin ) {
		if ( ! self::is_installed( $plugin ) ) {
			return array(
				'success' => false,
				'data'    => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'marketo' ), $plugin['name'] )
				)
			);
		}
	...
	public static function bulk_activate( $plugins ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'marketo' ), WP_PLUGIN_DIR ),
			);
		}
	...
	private static function api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'marketo' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'marketo' ), wp_remote_retrieve_body( $request ) );
				}
			}
	...
	public static function is_active( $plugin ) {
		//Check if the functions exists, in case the class is used in frontend
...
class FW_Auto_Install_Upgrader_Skin extends WP_Upgrader_Skin {
...
class FW_Auto_Install {
	public function __construct() {
		if ( current_user_can( 'install_plugins' ) ) {
			new FW_Super_Admin_Auto_Install();
		} elseif ( current_user_can( 'activate_plugins' ) && current_user_can( 'switch_themes' ) ) {
			new FW_Admin_Auto_Install();
		} else {
			new FW_Simple_Auto_Install();
		}
	}
}
