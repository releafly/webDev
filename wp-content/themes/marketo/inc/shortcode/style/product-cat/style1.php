    ...
    <?php  if(is_array($product_tab) && count($product_tab) > 0): ?>
        ...
        <ul class="nav nav-tabs xs-nav-tab" role="tablist">
            <?php foreach($product_tab as $key => $product_tabs): ?>
                ...
                ?>
                <li class="nav-item">
                    <a class="nav-link <?php echo esc_attr($active) ?>"  data-toggle="tab" href="#<?php echo esc_attr($rand_id.$key.'-'.sanitize_title($cat_name->slug)); ?>" role="tab" ><?php echo esc_html($cat_name->name); ?></a>
                </li>
...
<div class="tab-content">
    <?php if(is_array($product_tab) && count($product_tab) > 0): ?>
        <?php foreach($product_tab as $key => $tabs_content): ?>
            ...
            ?>
            <div class="tab-pane fade <?php echo esc_attr($active) ?>" id="<?php echo esc_attr($rand_id.$key.'-'.sanitize_title($cat_name->slug)); ?>" role="tabpanel">
