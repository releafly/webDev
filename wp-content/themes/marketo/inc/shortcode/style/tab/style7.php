...
<div class="xs-content-header content-header-v2">
    <?php  if(is_array($product_tab) && count($product_tab) > 0): ?>
        ...
        <ul class="nav nav-tabs xs-nav-tab version-3" role="tablist">
            ...
            ?>
            <li class="nav-item">
                <a class="nav-link <?php echo esc_attr($active) ?>"  data-toggle="tab" href="#<?php echo esc_attr($rand_id.$key.'-'.sanitize_title($product_tabs['product_title'])); ?>" role="tab" ><?php echo esc_html($product_tabs['product_title']); ?></a>
...
<div class="tab-content">
    <?php if(is_array($product_tab) && count($product_tab) > 0): ?>
        <?php foreach($product_tab as $key => $tabs_content): ?>
            ...
            ?>
            <div class="tab-pane fade <?php echo esc_attr($active) ?>" id="<?php echo esc_attr($rand_id.$key.'-'.sanitize_title($tabs_content['product_title'])); ?>" role="tabpanel">
                <?php
                printf(
                            ...
                            <?php endif; ?>
                                    <div class="xs-product-widget media version-2">
                                        ...
                                        <a class="xs_product_img_link" href="<?php echo esc_url(get_the_permalink()) ?>">
                                            <img class="d-flex" src="<?php echo esc_url($img_link); ?>"  alt="<?php the_title_attribute(); ?>">
                                        </a>
                                            <?php endif; ?>
