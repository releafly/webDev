                ...
                <?php foreach ($product_tab as $key => $product_tabs): ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link <?php echo esc_attr($active) ?>" data-toggle="tab"
                           href="#<?php echo esc_attr($rand_id . $key . '-' . sanitize_title($product_tabs['product_title'])); ?>"
    ...
    <?php if (is_array($product_tab) && count($product_tab) > 0): ?>
        <?php foreach ($product_tab as $key => $tabs_content): ?>
            ...
            <div class="tab-pane fade <?php echo esc_attr($active) ?>"
                 id="<?php echo esc_attr($rand_id . $key . '-' . sanitize_title($tabs_content['product_title'])); ?>"
                 role="tabpanel">
                <div class="xs-tab-slider owl-carousel">
                    ...
                    if ($xs_query->have_posts()):
                        ...
                        <?php endif; ?>
                            <div class="xs-product-widget media <?php echo esc_attr($hover_effect); ?>">
                                <?php if (!empty($img_link)): ?>
                                    <a class="xs_product_img_link" href="<?php echo esc_url(get_the_permalink()) ?>">
                                        <img class="d-flex" src="<?php echo esc_url($img_link); ?>"
