        ...
        <?php  if(is_array($product_tab) && count($product_tab) > 0): ?>
            <ul class="nav nav-tabs xs-nav-tab" role="tablist">
                <?php foreach($product_tab as $key => $product_tabs): ?>
                    ...
                    ?>
                    <li class="nav-item">
                        <a class="nav-link <?php echo esc_attr($active) ?>"  data-toggle="tab" href="#<?php echo esc_attr($rand_id.sanitize_title($product_tabs['product_title'])); ?>" role="tab" ><?php echo esc_html($product_tabs['product_title']); ?></a>
                    </li>
                <?php endforeach; ?>
            </ul>
        <?php endif; ?>
...
<div class="tab-content">
    <?php if(is_array($product_tab) && count($product_tab) > 0): $j = 1;?>
        <?php foreach($product_tab as $key => $tabs_content): ?>
            ...
            ?>
            <div class="tab-pane fade <?php echo esc_attr($active) ?>" id="<?php echo esc_attr($rand_id.sanitize_title($tabs_content['product_title'])); ?>" role="tabpanel">
                <div class="row">
                    <?php
                    $xs_count = 1;
                    if($xs_query->have_posts()):
                        while ($xs_query->have_posts()) :
                            ...
                            $img_link = xs_resize( get_post_thumbnail_id(), 193, 153,true );
                            ...
                            <div class="col-lg-6">
                                <div class="xs-product-wraper highlight">
                                    ...
                                    <a class="xs_product_img_link" href="<?php echo esc_url(get_the_permalink()) ?>">
                                        <img  src="<?php echo esc_url($img_featured); ?>"  alt="<?php the_title_attribute(); ?>">
                                    </a>
                                    <?php endif; ?>
                                    <div class="media">
