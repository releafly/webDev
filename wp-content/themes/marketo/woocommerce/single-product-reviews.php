            ...
            <div id="comments">
                <h2 class="woocommerce-Reviews-title"><?php
                    if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
                        ...
                        printf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'marketo' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
                    } else {
            ...
            <?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
                ...
                <div id="review_form_wrapper">
                    <div id="review_form">
                        ...
                        $comment_form = array(
                            'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'marketo' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'marketo' ), get_the_title() ),
                            ...
                            'label_submit'  => esc_html__( 'Submit', 'marketo' ),
                            'logged_in_as'  => '',
                            'comment_field' => '',
                        );

                        if ( $account_page_url = wc_get_page_permalink( 'myaccount' ) ) {
                            $comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( esc_html__( 'You must be <a href="%s">logged in</a> to post a review.', 'marketo' ), esc_url( $account_page_url ) ) . '</p>';
                        }

                        if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
                            $comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'marketo' ) . '</label><select name="rating" id="rating" aria-required="true" required>
                        ...
                        $comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Your review', 'marketo' ) . ' <span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" required></textarea></p>';
